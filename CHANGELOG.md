...
- MCP tool results changed shape: untrusted values are quoted JSON literals, so `**Message:** Database connection failed` now reads `**Message:** "Database connection failed"`, and a PHP exception class renders with doubled backslashes. Anything parsing that output needs updating; agents reading it are unaffected
