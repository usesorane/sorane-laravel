...
test('batch job is dispatched when buffer reaches threshold', function (): void {
    ...
    // Add first item - should not dispatch
    $job1 = new SendEventToSoraneJob(['event_name' => 'event1']);
    $job1->handle($buffer);

    Queue::assertNothingPushed();

    // Add second item - should dispatch
    $job2 = new SendEventToSoraneJob(['event_name' => 'event2']);
    $job2->handle($buffer);

    Queue::assertPushed(SendBatchToSoraneJob::class, function ($job): bool {
        return $job->type === 'events';
    });
});
